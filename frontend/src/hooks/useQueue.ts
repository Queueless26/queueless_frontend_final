    ...
    const fetchTokens = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          if (userId) {
            const myToken = data.find(
              (t: Token) => t.user_identifier === userId,
            );
            if (myToken) {
              setToken(myToken);
            } else {
      ...
      .on(
        ...
        (payload) => {
          ...
          setQueueTokens((prev) => {
            ...
            if (eventType === "INSERT") {
              if (
                ["WAITING", "NEAR", "CONFIRMED", "CALLED", "SERVING"].includes(
                  newRecord.state,
                )
              ) {
                next.push(newRecord);
  ...
  const confirmPresence = async (lat: number, long: number) => {
    ...
    try {
      const res = await fetch(
        "https://queueless-backend-final.onrender.com/api/v1/presence/verify",
        {
          method: "POST",
