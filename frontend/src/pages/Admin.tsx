...
const Admin: React.FC = () => {
      ...
      try {
        ...
        try {
          const counterRes = await fetch(
            "https://queueless-backend-final.onrender.com/api/v1/admin/ensure-counter",
            {
              method: "POST",
  ...
  const callNext = async () => {
    ...
    try {
      const res = await fetch(
        "https://queueless-backend-final.onrender.com/api/v1/admin/call-next",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const cancelToken = async (id: string) => {
    ...
    await fetch(
      "https://queueless-backend-final.onrender.com/api/v1/admin/cancel-token",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const toggleService = async () => {
    ...
    try {
      const res = await fetch(
        "https://queueless-backend-final.onrender.com/api/v1/admin/toggle-service",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleQRScan = async (scannedText: string) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://queueless-backend-final.onrender.com/api/v1/admin/complete-token",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const activeToken = tokens.find(
    (t) => t.state === "CALLED" || t.state === "SERVING",
  );
              ...
              <Card className="relative overflow-hidden">
                ...
                <CardContent>
                  ...
                  <p className="text-center text-sm text-muted-foreground mt-4">
                    Scan the QR code on the user's screen to complete their
                    service.
                  </p>
                </CardContent>
              </Card>
          ...
          <div>
            ...
            <div className="text-muted-foreground flex items-center gap-2 mt-1">
              ...
              <Badge
                ...
                className={service?.status === "OPEN" ? "bg-green-500" : ""}
              >
                {service?.status}
              </Badge>
            </div>
              ...
              <div className="flex flex-col items-center gap-4">
                {activeToken ? (
                  <div className="text-center w-full bg-white p-6 rounded-xl border border-indigo-100 shadow-sm relative overflow-hidden">
                    ...
                    <div className="text-7xl font-black text-indigo-600 my-4 tracking-tighter">
                      #{activeToken.token_number}
                    </div>
                    <Badge
                      variant="outline"
                      className="text-lg py-1 px-6 border-indigo-200 text-indigo-700 bg-indigo-50 mb-6"
                    >
                      {activeToken.state}
                    </Badge>
