...
const Dashboard: React.FC = () => {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setServices((prev) => ({
              ...prev,
              [svc.organization_id]: (prev[svc.organization_id] || []).map(
                (s) => (s.id === svc.id ? svc : s),
              ),
            }));
  ...
  const handleCreateService = async (orgId: string) => {
    ...
    try {
      ...
      if (data) {
        ...
        await supabase
          .from("counters")
          .insert({ service_id: data.id, name: "Counter 1" });
  ...
  const claimOrphans = async () => {
    ...
    if (
      !confirm(
        "This will find all 'orphan' services (no organization) and assign them to your organization. Continue?",
      )
    )
      return;
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage your organization and services
            </p>
          </div>
          <Button
          ...
          {orgs.length === 0 ? (
            <Card>
              <CardContent className="pt-6 text-center text-muted-foreground">
                No organizations found. Please ask your administrator to create
                one.
              </CardContent>
            </Card>
          ) : (
            orgs.map((org) => (
              <motion.div key={org.id} layout transition={{ duration: 0.2 }}>
                <Card
                  className={`overflow-hidden transition-colors ${expandedOrg === org.id ? "border-purple-200 bg-purple-50/30" : "hover:border-purple-100"}`}
                >
                  <div
                            ...
                            <div className="mb-4 bg-white p-3 rounded-lg border shadow-sm flex flex-col gap-2">
                              <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                                  <MapPin
                                    className={`w-3 h-3 ${location ? "text-green-500" : "text-orange-300 animate-pulse"}`}
                                  />
                                  {location
                              ...
                              <div className="flex justify-end gap-2 mt-2">
                                ...
                                <Button
                                  size="sm"
                                  onClick={() => handleCreateService(org.id)}
                                  disabled={creating}
                                >
